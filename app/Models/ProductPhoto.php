...
class ProductPhoto extends Model
{
    ...
    public static function photosPath($productsId)
    {
        $path = self::PRODUCTS_PATH;
        return storage_path("{$path}/{$productsId}");
    }
    ...
    public function updateWithPhoto(UploadedFile $files) : ProductPhoto
    {

        try {
            self::uploadFiles($this->product_id, [$files]);
            \DB::beginTransaction();
            $this->deletePhoto($this->file_name);
            $this->file_name = $files->hashName();
            $this->save();
            \DB::commit();
            return $this;
